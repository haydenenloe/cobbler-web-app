...
const ContactNode = memo(({ data }: { data: ContactNodeData }) => {
  const { name, title, company, relationship_strength, onConnect, isConnectable, onClick } = data;

  return (
    <div 
      className={`relative px-4 py-2 shadow-lg rounded-lg border-2 cursor-pointer ${
        relationship_strength === 3 ? 'border-green-500 bg-green-50' :
        relationship_strength === 2 ? 'border-yellow-500 bg-yellow-50' :
    ...
    >
      ...
      {onConnect && (
        <button
          ...
          title={isConnectable ? "Connect to this contact" : "Cannot connect to self"}
